...
@Component({
  ...
  templateUrl: './audio-summarise.component.html',
  styleUrls: ['./audio-summarise.component.scss'],
  animations: [
    trigger('slideInOut', [
      transition(':enter', [
...
export class AudioSummariseComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  readonly acceptedTypes = 'audio/wav,audio/mp3,audio/mpeg,audio/aiff,audio/aac,audio/ogg,audio/flac';
  ...
  private _handleSummarySuccess(response: SummariseResponse): void {
    this.logger.log('Audio analysis successful:', response);
  ...
  private _fetchTokenCount(file: FileInfo): void {
    ...
    this.apiService.countAudioTokens(request)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (response) => {
        ...
        error: (error) => {
          ...
          if (file.duration) {
            const minutes = file.duration / 60;
            const estimatedTokens = Math.round(minutes * 100);
            this.tokenCount.set(estimatedTokens);
          } else {
            this.tokenCount.set(500);
          }
          this.isLoadingTokens.set(false);
        },
      });
